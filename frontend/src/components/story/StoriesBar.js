  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity style={styles.item} onPress={() => onOpenStory(item)} activeOpacity={0.85}>
        <View style={[styles.storyRing, item.has_unviewed ? styles.unviewed : styles.viewed]}>
          <Avatar uri={item.user?.avatar_url} username={item.user?.username} size={58} />
        </View>
        <Text style={styles.label} numberOfLines={1}>{item.user?.username}</Text>
      </TouchableOpacity>
    );
  };
...
const styles = StyleSheet.create({
  ...
  unviewed: { borderColor: Colors.primary },
  viewed: { borderColor: Colors.border },
